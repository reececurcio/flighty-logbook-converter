...
const AIRPORTS_URL = './airports.csv?v=3';
...
async function loadAirports() {
  let csvText;
  try {
    const response = await fetch(AIRPORTS_URL, {cache:'no-store'});
    if (!response.ok) throw new Error(`HTTP ${response.status}`);
    csvText = await response.text();
  } catch (e) {
    throw new Error('Could not open the bundled airport database. Make sure airports.csv was uploaded beside index.html.');
  }
  ...
  for (const a of parsed.data) {
    ...
    const record = {name:a.name, lat, lon, ident:a.icao, iata:a.code, gps:a.icao, timeZone:a.time_zone};
    for (const code of [a.code, a.icao, a.city_code]) {
      if (code && !map.has(code.toUpperCase())) map.set(code.toUpperCase(), record);
    }
  }
  if (map.size < 1000) throw new Error('Bundled airport database loaded incorrectly. Re-upload airports.csv.');
  return map;
}
...
function renderResults(rows, warnings) {
  ...
  $('previewBody').innerHTML = rows.slice(0,100).map(r=>`<tr class="${r.lateLanding?'late':''}"><td>${escapeHtml(r.date)}</td><td>${r.from}</td><td>${r.to}${r.actualTo!==r.to?` → ${r.actualTo}`:''}</td><td>${formatMinutes(r.totalMinutes)}</td><td>${formatMinutes(r.nightMinutes)}</td><td>${formatMinutes(r.picMinutes)}</td><td>${formatMinutes(r.sicMinutes)}</td><td>${r.dayLandings}</td><td>${r.nightLandings}</td><td class="small">${escapeHtml(r.depTz)} → ${escapeHtml(r.arrTz)}</td></tr>`).join('');
